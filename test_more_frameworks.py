...
# New frameworks to test
NEW_FRAMEWORKS = [
    ('stability', 'stability_test.py', 'pip install stability-sdk'),
    ('gpt4all', 'gpt4all_test.py', 'pip install gpt4all'),
    ('semantic_kernel', 'semantic_kernel_test.py', 'pip install semantic-kernel'),
    ('langflow', 'langflow_test.py', 'pip install langflow'),
]
...
def main():
    ...
    print("="*60)
    print(f"\nFrameworks to test: {len(NEW_FRAMEWORKS)}")
    for name, script, install in NEW_FRAMEWORKS:
        print(f"  - {name}")
    print()
    
    results = []
    
    for framework_name, test_script, install_cmd in NEW_FRAMEWORKS:
        print(f"\n{'='*60}")
